...
function ReactorCore({ reducedEffects }: { reducedEffects: boolean }) {
  ...
  return (
    <group>
      ...
      <Sparkles
        count={reducedEffects ? 72 : 180}
        scale={[7, 3.2, 6]}
        size={reducedEffects ? 2.1 : 3.2}
        color="#8ceeff"
        speed={reducedEffects ? 0.14 : 0.25}
      />
    </group>
  );
}
...
function AmbientGeometry({ reducedEffects }: { reducedEffects: boolean }) {
  ...
  return (
    <group ref={gridRef}>
      ...
      <Sparkles
        count={reducedEffects ? 28 : 90}
        scale={[6, 1.2, 4]}
        size={reducedEffects ? 1.2 : 2}
        color="#4d88ff"
        speed={reducedEffects ? 0.08 : 0.12}
      />
    </group>
  );
}
    ...
    <div className="absolute inset-0 z-0">
      <Canvas
        ...
        gl={{
          ...
          alpha: true,
          powerPreference: reducedEffects ? "default" : "high-performance",
        }}
        dpr={dpr}
        onCreated={({ gl }) => {
      ...
      >
        ...
        {!reducedEffects ? (
          <EffectComposer>
            ...
            <Bloom mipmapBlur intensity={1.2} luminanceThreshold={0.12} radius={0.82} />
